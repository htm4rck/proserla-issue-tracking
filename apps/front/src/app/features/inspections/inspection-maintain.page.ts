...
export class InspectionMaintainPageComponent implements OnInit {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    reportedPersonAge: [''],
    employerType:   [''],
    /** Área seleccionada — al cambiar auto-rellena leaderCode */
    ...
    reportSource:   [''],
    correctiveMeasures: [''],
    /** Se rellena automáticamente al seleccionar areaCode */
    leaderCode:     [''],
    assignedTo:     [''],
  ...
  ngOnInit(): void {
    // Al cambiar el área, buscar el líder correspondiente
  ...
  /**
   * Dado un areaCode, busca en leader_areas el líder activo asignado a esa área.
   * Si hay varios, toma el que tiene isPrimary=true; si no, el primero.
   */
  private resolveLeaderForArea(areaCode: string): void {
    ...
    // Buscar en todas las áreas de todos los líderes activos
    const matches = this.leaders
      .filter(l => l.isActive && l.areas.some(a => a.areaCode === areaCode));
  ...
  // ── Áreas disponibles para el combo ──────────────────────────────────────
  ...
  private setupCreate(): void {
    ...
    // Pre-seleccionar área del usuario en sesión
  ...
  private preselectAreaFromSession(): void {
    ...
    // Solo pre-seleccionar si hay un líder activo para esa área
    const hasLeader = this.leaders.some(
      l => l.isActive && l.areas.some(a => a.areaCode === u.areaCode),
    );
    if (hasLeader) {
      this.form.controls.areaCode.setValue(u.areaCode);
    }
  }
  ...
  private applyInspection(data: Inspection): void {
    ...
    // Verificar si el líder sigue activo y asignado a esa área
    ...
    if (knownLeader) {
      ...
      // Líder histórico — mostrar referencia pero no bloquear
